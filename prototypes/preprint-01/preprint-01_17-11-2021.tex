...
\begin{document}
	...
	Now let $(M, \w, T^{n}, \mu)$ be an $2m$-dimensional symplectic toric orbifold The critical points for $mu$ are those in $M$ of which have a non-trivial stabiliser. Those points which are stabilised by a given subgroup $H \leq T^{m}$ are mapped under $\mu$ into a finite union of planes in $(\mft^{m})^{\ast}$, of which each normal vector is parallel to the annihilator of $\mfh$ in $(\mft^{n})^{m}$.
