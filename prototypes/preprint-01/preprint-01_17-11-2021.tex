...
\begin{document}
    ...
    \begin{defn}
        ...
        Let $\mcF = \{F_{1}, \ldots, F_{n}\}$ denote the set of faces of $\Delta_{M}$. For each $(n-d)$-dimensional face $F_{i} \subset \Delta_{M}$, the suborbifold $N_{i} := \mu^{-1}(F_{i})$ is a $2d$-dimensional toric symplectic suborbifold, with moment map $\phi_{i} := \restr{\mu}{N_{i}}
    \end{defn}
