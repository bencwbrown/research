...
\begin{document}
    ...
    \begin{defn}
        ...
        Let $\mcF = \{F_{1}, \ldots, F_{n}\}$ denote the set of faces of $\Delta_{M}$. For each $(n-d)$-dimensional face $F_{i} \subset \Delta_{M}$, the suborbifold $N_{i} := \mu^{-1}(F_{i})$ is a $2d$-dimensional toric symplectic suborbifold. If $i : N_{i} \hookrightarrow M$ denotes the inclusion, then the symplectic form for $N_{i}$ is $i^{\ast}\w$, with the moment map $\phi_{i} := \restr{\mu}{N_{i}}$, whose Hamiltian torus action is generated from the Lie algebra,
        \[
            \mft_{i}^{d} := \mft^{n} / \Span_{\RR}\Set{ u_{j} \in \mft^{n} | u_{j} \text{ is normal to } F_{i} } \cong \mft^{n}/\mft^{n-d}.
        \]
    \end{defn}
