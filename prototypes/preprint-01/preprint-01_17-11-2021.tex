...
\begin{document}
    ...
    Let $(M, \w, \mu, T^{n})$ be a $2n$-dimensional toric symplectic orbifold, with a Hamiltonian action of $T^{n}$ on $M$ with moment map $\mu : M \ra (\mft^{n})^{\ast}$. Denote by $\mu(M) = \Delta_{M} \subseteq (\mft^{n})^{\ast}$ the corresponding convex rational simple polytope. Let $\mcF = \{F_{1}, \ldots, F_{n}\}$ denote the set of faces of $\Delta_{M}$. For each $(n-d)$-dimensional face $F_{i} \subset \Delta_{M}$, which can be written as the intersection of $d$ facets,
     \[
        F_{i} = \bigcap_{F_{i} \subseteq H_{j}} H_{j}
    \]
    the suborbifold $N_{i} := \mu^{-1}(F_{i})$ is a $2d$-dimensional toric symplectic suborbifold. If $i : N_{i} \hookrightarrow M$ denotes the inclusion, then the symplectic form for $N_{i}$ is $i^{\ast}\w$, with the moment map $\phi_{i} := \restr{\mu}{N_{i}}$, whose Hamiltonian torus action is generated from the Lie algebra,
     \[
