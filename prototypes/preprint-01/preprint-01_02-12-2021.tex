...
\newcounter{subdefinition}[theorem]
...
\date{Version of the 2nd December 2021}	% Here you can change the date presented in the paper title
%\date{} 					% Or removing it
...
\begin{document}
   ...
   \end{equation}
    ...
    The reason introducing these are the following theorems:
    
    \begin{theorem}[\cite{Del88}]\label{thm:manifold-classification}
        Let $(X, \w, \mu, T^{d})$ be a symplectic toric manifold, that is, a connected $d$-dimensional manifold $X$ with symplectic form $\w$, and where $\mu : X \ra (\mft^{d})^{\ast}$ is a moment map for an effective Hamiltonian action of the torus $T^{d}$ on $X$, such that $\dim_{\CC} = d$. Then to every compact symplectic toric manifold, there corresponds a Delzant polytope $\Delta$ in $(\mft^{d})^{\ast}$, given by the image of the moment map $\mu(X)$.
        \newline\\
        Conversely, every Delzant polytope arises from some compact symplectic toric manifold.
    \end{theorem}

	\begin{example}
		Here, we consider a standard example where $n = 3, k = 1$, and $d = 2$. Consider $T^{3}$ acting on $\CC^{3}$ diagonally and, letting $u_{1} = (1,0), u_{2} = (0,1)$, and $u_{3} = (-1,-1)$, the map $\pi : \mft^{3} \ra \mft^{2}$ is given by the matrix
		\[
		\pi = \begin{bmatrix} 1 & 0 & -1 \\ 0 & 1 & -1 \end{bmatrix}.
		\]
		The kernel of $\pi$ is given by the Lie algebra $\mfk \cong \mft^{1} \subseteq \mft^{3}$ spanned by the vector $(1,1,1)$ in $\mft^{3}$. Therefore, the $K$-action on $\CC^{3}$ with respect to which we take the K\"ahler quotient is
		\[
			\imath(t) \cdot (z_{1}, z_{2}, z_{3}) = (tz_{1}, tz_{2}, tz_{3}),
		\]
		which, being effective and Hamiltonian, has moment map
		\[
			\mu(z) = (\imath^{\ast}\circ \phi)(z) = \frac{1}{2} \sum_{i=1}^{3}(|z_{i}|^{2} - \lambda_{i}) \in \mfk^{\ast},
		\]
		where $\lambda = (\lambda_{1}, \lambda_{2}, \lambda_{3}) \in (\mft^{3})^{\ast}$ is an arbitrary constant. Taking $\lambda = (0, 0, a)$, the symplectic quotient $X = \mu^{-1}(0)/K$ is
		\[
			X = \mu^{-1}(0)/K \cong \Set{ (z_{1}, z_{2}, z_{3}) \in \CC^{3} | \|z\|^{2} = 2a }/S^{1} \cong S^{5}/S^{1} \cong \CC\PP^{2},
		\]
		and has the residual action of $T^{3}/K \cong T^{2}$, again acting in an effective and Hamiltonian fashion. Its moment map $\mubar$ is the one for $T^{2}$ acting on $\CC\PP^{2}$, albeit with K\"ahler form given by the Fubini-Study form scaled by $c$;
		\[
			\mubar : X \ra (\mft^{2})^{\ast},\quad \mubar([z_{1}:z_{2}:z_{3}]) = \frac{c}{2} \left(\frac{|z_{1}|^{2}}{\|z\|^{2}}, \frac{|z_{2}|^{2}}{\|z\|^{2}}\right).
		\]
		Finally, the image $\mubar(\CC\PP^{2})$ in $(\mft^{2})^{\ast}$ is the $2$-simplex, scaled by $c$.
	\end{example}

	\begin{example}
		And now in this example, we consider the example where $n = 4, k = 2$, and $d = 2$. Consider again $T^{4}$ acting on $\CC^{4}$ diagonally and, and let $u_{1} = (1,0), u_{2} = (0,1), u_{3} = (-1,-1)$, and $u_{4} = (0,-1)$. This time, the map $\pi : \mft^{3} \ra \mft^{2}$ is 
		\[
		\pi = \begin{bmatrix} 1 & 0 & -1 & 0 \\ 0 & 1 & -1 & -1 \end{bmatrix}.
		\]
		Its kernel is generated by the vectors $(1, 1, 1, 0)$ and $(0, 1, 0, 1)$ in $\mft^{4}$, hence generate the Lie algebra $\mfk \cong \mft^{2} \subseteq \mft^{4}$. So, the $K$-action on $\CC^{4}$ in this case is
		\[
		\imath(t_{1}, t_{2}) \cdot (z_{1}, z_{2}, z_{3}, z_{4}) = (t_{1}z_{1}, t_{1}t_{2}z_{2}, t_{1}z_{3}, t_{2}z_{4}),
		\]
		which, also being effective and Hamiltonian, has the moment map
		\[
		\mu(z) = (\imath^{\ast}\circ \phi)(z) = \frac{1}{2}\left( |z_{1}|^{2} + |z_{2}|^{2} + |z_{3}|^{2}, |z_{2}|^{2} + |z_{4}|^{2} \right) - \left(\lambda_{1} + \lambda_{2} + \lambda_{3}, \lambda_{2} + \lambda_{4}\right) \in \mfk^{\ast}.
		\]
		where $\lambda = (\lambda_{1}, \lambda_{2}, \lambda_{3}, \lambda_{4}) \in (\mft^{4})^{\ast}$.
		
		Now if we take $\lambda = (0, 0, a, b)$, we can take the symplectic quotient $X = \mu^{-1}(0)/K$,
		\[
			X = \mu^{-1}(0)/K = \Set{(z_{1}, z_{2}, z_{3}, z_{4}) \in \CC^{4} | \begin{aligned}
					|z_{1}|^{2} + |z_{2}|^{2} + |z_{3}|^{2} &= 2a \\
					|z_{2}|^{2} + |z_{4}|^{2} &= 2b
				\end{aligned} 
			}/K
		\]
		
		\nb{Fill in identification with Hirzebruch surface.}
		
	\end{example}

	\begin{center}
		\begin{tikzpicture}[scale=1.00] %[x={10.0pt},y={10.0pt}]
			\draw[step=1.5cm,gray,very thin] (-0.5,-0.5) grid (3.5,3.5);
			\filldraw[fill=gray!10!white, draw=black] (0,0) -- (3,0) -- (0,3) -- cycle;
		\end{tikzpicture}% pic 1
		\qquad % <----------------- SPACE BETWEEN PICTURES
		\begin{tikzpicture}[scale=1.00] %[x={10.0pt},y={10.0pt}]
			\draw[step=1.5cm,gray,very thin] (-0.5,-0.5) grid (6.5,3.5);
			\filldraw[fill=gray!10!white, draw=black] (0,0) -- (6,0) -- (3,3) -- (0,3) -- cycle;
		\end{tikzpicture}% pic 2        
	\end{center}

    \begin{theorem}[\cite{Lerman1997}]\label{thm:orbifold-classification}
        Let $(X, \w, \mu, T^{d})$ be a symplectic toric orbifold, that is, a connected $d$-dimensional orbifold $X$ along with the same symplectic form $\w$, moment map $\mubar : X \ra (\mft^{d})^{\ast}$, and effective Hamiltonian action of the torus $T^{d}$ on $X$, as in Theorem (\ref{thm:manifold-classification}. Then to every compact symplectic toric orbifold, there corresponds a labelled polytope $\Delta$ in $(\mft^{d})^{\ast}$, similarly given by the image of the moment map $\mu(X)$. That is, the image $\mu(X)$ is a rational simple polytope and, for each open facet $\open{F}$ of $\mu(X)$, there exists a positive integer $n_{\open{F}}$ such that the structure of every $x \in \mu^{-1}(\open{F})$ is $\ZZ/n_{\open{F}}\ZZ$.
        \newline\\
        Conversely, every labelled polytope arises from some compact symplectic toric orbifold.
    \end{theorem}
    
    \begin{example}

    \nb{Fill in example for weighted projective space.}
    ...
    \begin{center}
    	\begin{tikzpicture}[scale=1.00] %[x={10.0pt},y={10.0pt}]
    		\draw[step=1.5cm,gray,very thin] (-0.5,-0.5) grid (6.5,3.5);
    		\filldraw[fill=gray!10!white, draw=black] (0,0) -- (6,0) -- (0,3) -- cycle;
    	\end{tikzpicture}% pic 1   
    \end{center}
	...
	\begin{align}
		\mubar_{\HK}[z,w] = \mubar_{\RR}[z,w] \oplus \mubar_{\CC}[z,w] &= \half \sum_{i=1}^{n}(|z_{i}|^{2} - |w_{i}|^{2} - l_{i})\e_{i} \oplus \sum_{i=1}^{n}(z_{i}w_{i})\e_{i} \\
