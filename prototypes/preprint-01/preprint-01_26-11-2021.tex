...
\begin{document}
    ...
    There are a finite number of subtori $H_{i}$ which can appear as stabliser subgroups of points in $M$, with each fixed-point set having a finite number of components. Denote the set of these components by $\mcF = \{F_{1}, \ldots, F_{n} \}$, and label their respective stabliser subtori by $\{H_{1}, \ldots, H_{n}\}$.
