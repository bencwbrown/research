...
\begin{document}
   ...
   \subsection{Below - Old Content} 
	...
	Let $(M, \w)$ be a compact symplectic manifold, and assume that the class of the symplectic form is integral, \ie $\tfrac{1}{2\pi}[\w] \in H^{2}(M; \ZZ)$. This integrality condition is equivalent to the existence of a complex line bundle $\mcL \ra M$ with a Hermitian structure, such that its $1$\textsuperscript{st} Chern class $c_{1}(\mcL)$ equals the class of the symplectic form $\tfrac{1}{2\pi}[\w]$ \cite{Duistermaat2011}.
