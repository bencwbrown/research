...
\begin{document}
   ...
   \end{equation}
    ...
    \begin{theorem}\label{thm:orbifold-classification}
        Let $(X, \w, \mubar, T^{d})$ be a symplectic toric orbifold, that is, a connected orbifold $X$ along with the same symplectic form $\w$, moment map $\mubar : X \ra (\mft^{d})^{\ast}$, and effective Hamiltonian action of the torus $T^{d}$ on $X$, as in Theorem (\ref{thm:manifold-classification}. Then to every compact symplectic toric orbifold, there corresponds a Delzant polytope $\Delta$ in $(\mft^{d})^{\ast}$, given by the image of $X$ under the moment map $\mubar$.
    \end{theorem}
