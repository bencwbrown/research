...
\begin{document}
   ...
   \subsection{Below - Old Content} 
    ...
    \]
    From $\w$ and $J$, we also have a Hermitian structure of each of the $\Omega^{0,j}$'s, so that the adjoint operator $\dbar_{\mcL}^{\ast}$ is well-defined. The operator $\slashed{\partial}_{\CC} = \dbar_{\mcL} + \dbar_{\mcL}^{\ast}$ is known as the spin-$\CC$ Dirac operator and, as $M$ is compact,
