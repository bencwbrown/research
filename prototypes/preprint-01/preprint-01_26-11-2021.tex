   ...
   \end{equation}
    ...
    \begin{definitions}\label{defn:polytope-properties}
        Suppose that $\Delta$ is a convex polytope in $\RR^{d}$. We say that it is:
            \begin{enumerate}
                \item \emph{simple}, if there are $d$ edges meeting at each vertex;
                
                \item  \emph{rational}, if the edges meeting at the vertex $v$ are of the form, $v + ta_{i}$, with $t \geq 0$ and $a_{i} \in \ZZ^{d}$, for $1 \leq i \leq d$. \Ie that the edges to the vertex $v$ emanate out in a direction given by the integral vectors, $a_{i}$;
                
                \item \emph{smooth}, if for each vertex, the corresponding edge vectors $a_{1}, \ldots, a_{d}$ form a $\ZZ$-basis of $\ZZ^{d}$;
