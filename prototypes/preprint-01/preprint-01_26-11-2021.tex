...
\begin{document}
	...
	\begin{figure}[h!]
		...
		\begin{tikzpicture}
			...
			\draw (-1.5,0) -- (5,0);
			\draw[thick,->] (1.5,0) -- (1.5,0.5) node[anchor=north west] {$u_{2}$};
			\draw (0,-1.5) -- (0,5);
			\draw[thick,->] (0,1.5) -- (0.5,1.5) node[anchor=south west] {$u_{1}$};
			\draw (4.5,-1.5) -- (-1.5,4.5);
			\draw[thick,->] (1.5,1.5) -- (1.146,1.146);
		\end{tikzpicture}
    ...
    \]
    Set $\tilde{F}_{i} := \mu_{i}^{-1}(\lambda_{i}) \subset M$, so that $\mu_{i}(\tilde{F}_{i}) = F_{i}$. 
    
     Restricted to the face $F_{i}$, we see that $\restr{\mu_{i}}{F_{i}} = \lambda_{i}$, hence is constant along $\tilde{F}_{i} := \mu_{i}^{-1}(\lambda_{i})$. Therefore the derivative $d_{p}\mu_{i}$ vanishes for all $p \in \tilde{F}_{i}$, whence $F_{i}$ is the annihilator in $\mfg^{\ast}$ of the Lie algebra of the stabiliser, $\mfg_{F_{i}} = \Lie\left(\Stab_{G}(F_{i})\right)$. What this means is that the fundamental vector field generated by $u_{i} \in \mfg$ vanishes along $F_{i}$, 
    
=======
    where the $u_{i} \in \mfg$ are the inwards-pointing normal vectors to the half-spaces that delineate the polytope, and each $\lambda_{i} \in \mathbb{R}$.
