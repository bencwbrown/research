...
\begin{document}
   ...
   \subsection{Below - Old Content} 
    ...
    \[
        ...
        \end{tikzcd}
    \]
    Now, given the holomorphic line bundle $\mcL$ froma bove with its holomorphic connection $\nabla$, we define a \emph{twisted Dolbeault complex} by setting $\dbar_{\mcL} = \dbar \otimes 1 + 1 \otimes \nabla$, which defines the following complex for $\mcL$-valued forms
    \[
    \begin{tikzcd}[ampersand replacement=\&]
        0 \ar[r] \& \mcC^{\infty}(\wedge^{0,0} \otimes \mcL) \ar[r, "\dbar"] \& \mcC^{\infty}(\wedge^{0,1} \otimes \mcL) \ar[r, "\dbar"] \& \ldots \ar[r, "\dbar"] \& \mcC^{\infty}(\wedge^{0,2n} \otimes \mcL) \ar[r, "\dbar"] \& 0.
    \end{tikzcd}
    \]
