...
\begin{document}
   ...
   \end{equation}
    ...
    \begin{theorem}\label{thm:delzant-classification}
        Let $(X, \w, \mubar, T^{d})$ be a symplectic toric manifold, that is, a connected manifold$X$ with symplectic form $\w$, and where $\mubar : X \ra (\mft^{d})^{\ast}$ is a moment map for an effective Hamiltonian action of the torus $T^{d}$ on $X$, such that $\dim_{\CC} = d$. 
    \end{theorem}
