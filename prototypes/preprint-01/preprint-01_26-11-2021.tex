...
\begin{document}
    ...
    Provided that $0$ is a regular value of the moment map (\ref{eqn:K-moment-map}), we obtain a smooth K\"ahler quotient $X = \mu^{-1}(0)/K$ which is also a toric variety. The residual torus $T^{d} = T^{n}/K$ from (\ref{eqn:lie-group-seq}) 
