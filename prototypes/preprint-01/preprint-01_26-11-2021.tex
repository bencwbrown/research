...
\begin{document}
   ...
   \end{equation}
    ...
    \begin{definition}\label{defn:polytope-properties}
        ...
        \begin{subdefinition}
            \textbf{simple}, if there are $d$ edges meeting at each vertex;
        \end{subdefinition}
    
        \begin{subdefinition}
            \textbf{rational}, if the edges meeting at the vertex $v$ are of the form, $v + ta_{i}$, with $t \geq 0$ and $a_{i} \in \ZZ^{d}$, for $1 \leq i \leq d$. \Ie, the edges to the vertex $v$ emanate out in a direction given by the integral vectors, $a_{i}$;
        \end{subdefinition}
        
        
        \begin{subdefinition}
            \textbf{smooth}, if for each vertex, the corresponding edge vectors $a_{1}, \ldots, a_{d}$ form a $\ZZ$-basis of $\ZZ^{d}$;
        \end{subdefinition}
    
        \begin{subdefinition}
            \textbf{Delzant}, if the polytope is simultaneously simple, rational, and smooth.
        \end{subdefinition}    
    \end{definition}
