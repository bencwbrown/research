...
\begin{document}
	...
	\nb{Section last edited on 14th October 2021}
		
	\nb{Reference: Maxence Mayrand,  Nahm equations in hyperk{\"a}hler geometry, Propositions 4.9 and 4.10}
	...
	\begin{proof}
		...
		\[
			\Phi : M \times K_{\CC} \times \mfk_{\CC}^{\ast} \lra \mfk_{\CC}^{\ast}, \qquad \Phi(m, k, \lambda) = \mu(m) - \Ad_{k}^{\ast}(\lambda),
		\]
		thus
		\[
		...
		\]
 		...
 		To prove that $\bar{\jmath}$ is a symplectomorphism, first observe that the map $m \mapsto (e, \mu(m))$ sends $M$ into the fibre $T_{e}^{\ast}K_{\CC} \cong \mfk_{\CC}^{\ast}$ which is a Lagrangian submanifold, as any fibre of a cotangent bundle is. Hence $i^{\ast}(\w + \nu) = i^{\ast}\w$ when restricted to the image $j(M) \subseteq M \times T_{e}^{\ast}K_{\CC}$. Also, $j^{\ast}i^{\ast}(\w + \nu) = \w$, and by the Marsden-Weinstein reduction theorem, we have $q^{\ast}\eta = i^{\ast}(\w + \nu)$, implying that $\w = j^{\ast}i^{\ast}(\w + \nu) = j^{\ast}q^{\ast}\eta = (j \circ q)^{\ast}\eta$. However, $\bar{\jmath} = q \circ j$, so $\bar{\jmath}^{\ast}\eta = \w$ and $\bar{\jmath}$ is also a symplectomorphism.
