...
\begin{document}
	...
	Applied to our case, the collection of vectors $\{u_{j}\}$ in $\mft^{d}$ and choice of lift $\lambda \in (\mft^{n})$ correspond to a hyperplane arrangement $\mcA = \{H_{1}, \ldots, H_{n}\}$ of hyperplanes in $(\mft^{d})^{\ast}$, with each $H_{j} \subset (\mft^{d})^{\ast}$ a weighted, cooriented, affine hyperplane
