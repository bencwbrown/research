...
\begin{document}
	...
	The data used to construct a hypertoric orbifold $M$ can be encoded as the data describing a \emph{hyperplane arrangement} in $(\mft^{d})^{\ast} \cong \RR^{d}$. The collection of non-zero vectors $\{u_{j}\}$ in $(\mft^{d})^{\ast}$
