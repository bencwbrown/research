...
\begin{document}
	...
	The data used to construct a hypertoric orbifold $M$, consisting of the collection of non-zero vectors $\{u_{j}\}$ in $(\mft^{d})^{\ast}$ and an element $\lambda \in \mfk^{\ast}$, can be encoded into a \emph{hyperplane arrangement} in $(\mft^{d})^{\ast} \cong \RR^{d}$. The 
