...
\begin{document}
	...
	\begin{defn}
		...
		\begin{enumerate}
			...
			\item[(cooriented) --] Let 
			\[
				H^{\pm} = \Set{x \in \RR^{d} | \langle x,\, \pm u \rangle = 0} = H.
			\]
			A \emph{coorientation} for $H$ is a choice of orientation such that $H^{+} = H$, or $H^{-} = H$. A hyperplane $H$ is \emph{cooriented} if it has a coorientation.
	...
	Applied to our case, the collection of vectors $\{u_{j}\}$ in $\mft^{d}$ and choice of lift $\lambda \in (\mft^{n})$ correspond to a hyperplane arrangement $\mcA = \{H_{1}, \ldots, H_{n}\}$ of hyperplanes in $(\mft^{d})^{\ast}$, with each $H_{j} \subset (\mft^{d})^{\ast}$ a weighted, cooriented, affine hyperplane given by
	\[
		H_{j} = \Set x \in (\mft^{d})^{\ast} | \langle x,\, u_{j} = \lambda_{j}
	\]
