...
\begin{document}
	...
	Applied to our case, the collection of vectors $\{u_{j}\}$ in $\mft^{d}$ and choice of lift $\lambda \in (\mft^{n})$ correspond to a hyperplane arrangement $\mcA = \{H_{1}, \ldots, H_{n}\}$ of weighted, cooriented, affine hyperplanes $H_{j}}$ in $(\mft^{d})^{\ast}$, given by
	\[
