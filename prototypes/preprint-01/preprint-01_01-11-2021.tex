...
\begin{document}
	...
	Let us now specialise to the case when the Lie group $G$ is the $n$-dimensional torus, $T^{n}$; it acts on $\CC^{n}$ via the diagonal left-multiplication and, after fixing the complex structure $J_{1}$, on $\HH^{n}$ via its identification with $T^{\ast}\CC^{n}$, with the natural torus action induced from that on $\CC^{n}$. On the other hand, fixing $J_{2}$ in order to identify $\HH^{n} \cong \CC^{n} \times \CC^{n}$, the $T^{n}$-action on $\CC^{n} \times \CC^{n}$ can be written as
	\[
			t \cdot (z,w) = (tz,t^{-1}w).
	\]
	This action is hyper-Hamiltonian, with three associated moment maps $\mu_{1}, \mu_{2}, \mu_{3}$ for each complex structure, that can be written as
	\[
		\mu_{1}(z,w) = \frac{1}{2}\sum_{j=1}^{n} \left(|z_{j}|^{2} - |w_{j}|^{2}\right)e_{j} + \lambda_{1} \in (\mft^{n})^{\ast},
	\]
	\[
		(\mu_{2} + i\mu_{3})(z,w) = \sum_{j=1}^{n}(z_{j}w_{j})e_{j} + (\lambda_{2} + i \lambda_{3}) \in (\mft_{\CC}^{n})^{\ast}.
	\]
