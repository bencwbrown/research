...
\begin{document}
	...
	\]
	Observe that if we choose a different lift $\tilde{\lambda} \in (\mft^{n})^{\ast}$ such that $\lambda - \tilde{\lambda} \in \ker(\imath^{\ast})$, then this corresponds to translating the arrangement $\mcA$ inside of $(\mft^{d})^{\ast}$, and  Such a hyperplane introduces the following two half-spaces in $(\mft^{d})^{\ast}$
	\begin{align*}
