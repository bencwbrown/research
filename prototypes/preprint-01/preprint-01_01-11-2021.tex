...
\begin{document}
	...
	\]
	is called a \emph{hypertoric variety}. Both of these have a residual $T^{d}$-action which is effective, and also Hamiltonian in the case of $X$, hyper-Hamiltonian in the case of $M$. When $K$ is not connected, \ie when the vectors $\{u_{j}\}_{j=1}^{n}$ \emph{do not} span $\mft_{\ZZ}^{d}$ over the integers, then $X$ and $M$ are toric and hypertoric \emph{orbifolds} respectively, instead. In the latter orbifold case, $K$ is isomorphic to the product $K_{0} \times \Gamma$, where $K_{0}$ is the connected component of $K$ generated by the identity, hence abelian so $K_{0}$ is now a torus, and $\Gamma$ is a finite abelian group.
