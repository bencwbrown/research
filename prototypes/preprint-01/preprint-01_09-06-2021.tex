...
\begin{document}
	...
	\begin{proof}
		...
		Now define $j : M \rightarrow \Phi^{-1}(0) \subset M \times T^{\ast}K_{\CC}$ by $j(m) = (m, e, \mu(m))$, which descends to a biholomorphism $\bar{\jmath} : M \rightarrow \Phi^{-1}(0)/K_{\CC}$.
		 ...
		 By the Marsden-Weinstein reduction theorem, there exists a unique complex-symplectic form $\eta$ on $(M \times T^{\ast}K_{\CC}) \sslash_{0} K_{\CC}$ such that, if $\bar{\jmath}^{\ast}\eta = \w$, let $\nu$ denote the canonical form on $T^{\ast}K_{\CC}$ and let $i : \Phi^{-1}(0) \hookrightarrow M \times T^{\ast}K_{\CC}$ denote the inclusion, and $q : \Phi^{-1}(0) \rightarrow \Phi^{-1}(0)/K_{\CC}$ denote the quotient map. 
		
		\[
		\begin{tikzcd}
			(M, \w)\arrow[r, "j"] \arrow[dr, dotted, "\bar{\jmath}"] & \left( \Phi^{-1}(0), i^{\ast}(\w + \nu) \right) \arrow[d, "q"] \arrow[dr, hook, "i"] &  \\
			& (\Phi^{-1}(0) / G_{\CC}, \eta) & (M \times T^{\ast}G_{\CC}, \w + \nu )
		\end{tikzcd}
		\]
		
		By the Marsden-Weinstein reduction theorem, the form $\eta$ on 
	\end{proof}
