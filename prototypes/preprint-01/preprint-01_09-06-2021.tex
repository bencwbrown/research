...
\begin{document}
	...
	\begin{proof}
		The $K_{\CC}$-action on $T^{\ast}K_{\CC}$ is free and proper, and thus so is the diagonal action on the product $M \times T^{\ast}K_{\CC}$, so we are in the position to apply the holomorphic Marsden-Weinsten Reduction Theorem. Identifying via polar decomposition $T^{\ast}K_{\CC} \cong K_{\CC} \times (\mfk^{\ast} \otimes \CC)$, the moment map for the diagonal action is
		
		\[
			\Phi : M \times T^{\ast}K_{\CC} \lra \mfk^{\ast} \otimes \CC, \qquad \Phi(m, k, \lambda) = \mu(m) - \Ad_{g}^{\ast}(\lambda).
		\]
