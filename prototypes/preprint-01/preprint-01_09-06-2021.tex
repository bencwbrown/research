...
\begin{document}
	...
	\begin{proof}
		The $K_{\CC}$-action on $T^{\ast}K_{\CC}$ is free and proper, and thus so is the diagonal action on the product $M \times T^{\ast}K_{\CC}$, so we are in the position to apply the holomorphic Marsden-Weinsten Reduction Theorem. The moment map for the diagonal action is
