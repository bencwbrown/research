...
\begin{document}
	...
	\begin{proof}
		...
		To prove that $\bar{\jmath}$ is also a symplectomorphism, \ie if $\eta$ represents the complex-symplectic form on $(M \times T^{\ast}K_{\CC}) \sslash_{0} K_{\CC}$, that $\bar{\jmath}^{\ast}\eta = \w$, let $\nu$ denote the canonical form on $T^{\ast}K_{\CC}$ and let $i : \Phi^{-1}(0) \hookrightarrow M \times T^{\ast}K_{\CC}$ denote the inclusion, and $q : \Phi^{-1}(0) \rightarrow \Phi^{-1}(0)/K_{\CC}$ denote the quotient map.
		
		\[
		\begin{tikzcd}
			(M, \w)\arrow[r, "j"] \arrow[dr, dotted, "\bar{\jmath}"] & \left( \Phi^{-1}(0), (\w + \nu) \right) \arrow[d, "q"] \arrow[dr, hook, "i"] &  \\
