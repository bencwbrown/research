...
\begin{document}
	...
	\begin{proof}
		The map $m \mapsto (1, \Phi(m))$ sends $M$  to the Lagrangian $\mfk^{\ast} \subset T^{\ast}K$,  so $j$ pulls back the \HK forms$\w_{HK} + \restr{\w_{can}}{\mfk^{\ast}}$ on $M \times T^{\ast}K_{\CC}$ to $\w_{HK}$ on $M$. Thus $j$
	\end{proof}
