...
\begin{document}
	...
	\]
	which also has the structure of a hyperk\"ahler manifold provided that $G$ acts freely on $\mu_{\HK}^{-1}(\lambda)$:
	
	\begin{theorem}
		Let $M$ be a hyperk\"ahler manifold equipped with a hyper-Hamiltonian action of $G$, and corresponding moment map $\mu_{\HK} : M \ra \mfg^{\ast} \otimes \RR^{3}$. Suppose that $\lambda \in \mfg^{\ast} \otimes \RR^{3}$ is a regular value for $\mu_{\HK}$, and invariant under the coadoint action of $G$.
	\end{theorem}
