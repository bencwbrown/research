...
\begin{document}
	...
	Given a compact Lie group $G$, we say that an action of $G$ on $M$ is \emph{tri-symplectic} if it preserves each of the forms $\w_{i}$, which from the compatibility condition means that the action also preserves the metric $g$ and the complex structures $I, J$, and $K$; so the action is additionally \emph{isometric} and \emph{tri-holomorphic}.
