...
\begin{document}
	...
	Let $(M, \w, \mu)$ be a Hamiltonian $G$-manifold with moment map $\mu : M \ra \mfg^{\ast}$, where $G$ is a connected Lie group. Further assume that the class of the symplectic form is \emph{integral}, \ie $\tfrac{1}{2\pi}[\w] \in H^{2}(M; \ZZ)$. This integrality condition is equivalent to the existence of a complex line bundle $\pi : \mcL \ra M$ with a Hermitian structure, such that its $1$\textsuperscript{st} Chern class $c_{1}(\mcL)$ equals the class $\tfrac{1}{2\pi}[\w]
