...
\begin{document}
	...
	One can show that the level-set $\rho^{-1}$
	\[
		\rho^{-1}(\e) = \Set{(m, \xi) \in M \times \CC | }
	\]
	
	is equal to the the disjoint union $\Sigma_{1} \sqcup \Sigma_{2}$, where
	\begin{align*}
		\Sigma_{1} &= \Set{(m,z) \in M \times \CC | \mu(m) < \e,\ |\xi| = + \sqrt{\mu(m) + \e} }, \\
