...
\begin{document}
	...
	\end{align*}
	Furthermore, after quotienting $\rho^{-1}(\e)$ by the circle action, $\Sigma_{1}/S^{1}$ may be identified with $\Set{m \in M | \mu(m) < \e}$, whereas $\Sigma_{2}/S^{1}$ is just the usual symplectic quotient $\mu^{-1}(m)/S^{1}$. Hence, the symplectic cutting operation may be intuitively though of as ``discarding'' the piece $\Set{m | \mu(m) > \e}$ of $M$, ``keeping'' the piece $\Set{ m | \mu(m) < \e}$ of $M$, and collapsing the boundary piece $\mu^{-1}(\e)$ by quotienting out the circle action.
