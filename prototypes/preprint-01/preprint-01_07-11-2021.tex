...
\begin{document}
	...
	Globally, the $S^{1}$-action does not act as a subtorus of $T^{d}$ on $M$. However, when restricted to a extended core component $\mcE_{A}$, $S^{1}$ does act on $\mcE_{A}$ as a subtorus of $T^{d}$, and its inclusion $S^{1} \overset{j}{\hookrightarrow} T^{d}$ depends on the subset $A \subseteq \{1, \ldots, n\}$ in a combinatorial way.
