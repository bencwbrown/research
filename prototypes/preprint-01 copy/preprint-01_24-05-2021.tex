...
%			\label{fig:test1}
	...
	As the $S_{A}^{1}$-action depends combinatorially on the component $\mcE_{A}$, the image of the real moment map in $(\mft^{d})^{\ast}$ is compactified by inserting a half-space $Z_{A}$ with inwards-pointing normal $v_{A} = \sum_{i \in A}u_{i}$ determining the orientation, on each component $\Delta_{A}$.
	
	For each subset $A \subseteq \{1, \ldots, n\}$, the image of each extended core component $\mcE_{A}$ is a convex polytope,
	\[
		\Delta_{A} = \bar{\mu}_{\RR}(\mcE_{A}) = \lbracket \bigcap_{i\in A} F_{i} \rbracket \cap \lbracket \bigcap_{i\not\in A} G_{i} \rbracket,
	\]
	and taking the symplectic cut with respect to the vector $v_{A}$ corresponds to inserting a half-space, $\mc{H}_{A}$ say, where $\mc{H}_{A}$ is defined by
	\[
		\mc{H}_{A} = \Set{ x \in \Delta_{A} | \left\langle y,\, v_{A} \right\rangle \leq \epsilon }.
	\]
