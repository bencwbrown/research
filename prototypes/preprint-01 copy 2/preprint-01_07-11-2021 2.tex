...
\begin{document}
	...
	\begin{defn}
		The \emph{symplectic cut} $M_{\cut}^{\e}$ of $M$ is the symplectic reduction of the product $M \times \CC$ at level $\e$ with respect to the diagonal $S^{1}$-action,
		\[
			M_{\cut}^{\e} := (M \times \CC) \sslash_{\e} S^{1} = \rho^{-1}(\e)/S^{1}.
		\]
	\end{defn}
	...
	We will use the residual $S^{1}$-action to compactify the hypertoric manifolds, which bounds the $w$-terms of the real moment map $\bar{\mu}_{\RR}$ from above. This is reflected in the image of $\bar{\mu}_{\RR}$ by inserting inwards-point half-spaces in each of the polyhedra, $\Delta_{A}$ and, in particular, bounding those which were originally unbounded.
	
	To this end, for a hypertoric manifold $M$, let $S^{1}$ act on the product $M \times \CC$ diagonally,
	\[
		e^{i\theta} \cdot \big( [z,w], \xi   \big) = \left( e^{i\theta} \cdot [z,w], e^{i\theta}\xi\right) = \left( [z,e^{i\theta}w], e^{i\theta}\xi\right).
	\]
	This diagonal action is Hamiltonian, with moment map
	\[
		\Phi : \mf{M} \times \CC \ra \RR, \quad \Phi\left( [z,w], \xi  \right) = \phi[z,w] + |\xi|^{2} = \|w\|^{2} + |\xi|^{2}.
	\]
	For some $\e > 0$, the level-set $\Phi^{-1}(\e)$ decomposes into the disjoint union $\Phi^{-1}(\e) = \Sigma_{1} \sqcup \Sigma_{2}$, where
	\begin{align*}
		\Sigma_{1} &= \Set{([z,w],\xi) \in M \times \CC | \tfrac{1}{2}\|w\|^{2} < \e,\ |\xi| = + \sqrt{2\e - \|w\|^{2}}}, \\
		\Sigma_{2} &= \Set{([z,w], 0) \in M \times \CC | \tfrac{1}{2}\|w\|^{2} = \e},
	\end{align*}
	so, after taking the quotient by $S^{1}$, we get the cut hypertoric manifold,
	\[
		M_{\cut}^{\e} = \Set{[z,w]\in M | \tfrac{1}{2}\|w\|^{2} \leq \e}.
	\]
