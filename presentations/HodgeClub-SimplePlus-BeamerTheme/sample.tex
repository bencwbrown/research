...
\begin{frame}{... Orbifolds?}
		\begin{block}{Theorem (Kawasaki-Riemann-Roch)}
			...
			Then
			\[
			\chi(e^{\xi}) = \sum_{p \in M^{T}} \left( \frac{1}{|\Gamma_{p}|} \sum_{\gamma \in \Gamma_{p}} \frac{e^{\langle \mu(p),~\xi\rangle}}{\prod_{i = 1}^{n}\left(1 - e^{\langle \gamma,~\xi\rangle}e^{\langle \alpha_{i,p},~ \xi \rangle }\right)} \right),
			\]
