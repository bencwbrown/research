...
\begin{frame}{... Orbifolds?}
		\begin{block}{Theorem (Kawasaki-Riemann-Roch)}
			...
			Then
			\[
			\chi(e^{\xi}) = \sum_{p \in M^{T}} \left( \frac{1}{|\Gamma_{p}|} \sum_{\gamma \in \Gamma_{p}} \frac{e^{\langle \mu(p),~\xi\rangle}}{\prod_{i = 1}^{n}\left(1 - e^{2\pi \sqrt{-1}\langle \gamma,~\xi\rangle}e^{\langle \alpha_{i,p},~ \xi \rangle }\right)} \right),
			\]
...
\begin{frame}{$\CC\PP(1,2,1)$ Example}
	\begin{figure}[h!]
		...
		\scalebox{0.5}{
			\begin{tikzpicture}
				...
				\node at (0,7) {$\Gamma_{13} \cong \ZZ/2\ZZ$};
				% Normal vectors u_{i}
				...
				\draw[thick, ->] (12,0) -- (11,0.5) node[above right] {$\alpha_{1,p} = (-2, 1)$};
	...
	\end{figure}
	Point $p_{13}$ (mapped onto $v_{13}$) has local group $\Gamma_{13} \cong \ZZ/2\ZZ$; other two points are smooth.
	\hfill \\
	Contribution from $p_{13}$:
	\[
		\frac{1}{2}\left( \frac{y^{k}}{(1 - xy^{-1/2})(1 - y^{-1/2})} + \frac{y^{k}}{(1 - xy^{-1/2})(1 - y^{-1/2})} \right)
	\]
\end{frame}
