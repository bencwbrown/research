...
\begin{frame}{$\CC\PP(1,2,1)$ Example}
	...
	\end{figure}
	Point $p_{13}$ (mapped onto $v_{13}$) has local group $\Gamma_{13} \cong \{0\} \oplus \ZZ/2\ZZ$; other two points are smooth.
	\hfill \\
	Contribution from $p_{13}$:
	\[
		\frac{1}{2}\left( \frac{y^{k}}{(1 - xy^{-1/2})(1 - y^{-1/2})} + \frac{y^{k}}{(1 - e^{\pi\sqrt{-1}}xy^{-1/2})(1 - e^{\pi\sqrt{-1}}y^{-1/2})} \right)
	\]
\end{frame}
