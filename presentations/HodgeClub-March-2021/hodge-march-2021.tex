...
\subsection{The Equivariant Euler Class}
...
Now let $G = \U(n)$, then $\mfg = \mf{u}(n)$ can be identified with the space of matrices of the form $iA$, where $A = A^{T}$. Define the polynomial $c_{k}$, of degree $k$ in $A$ to be the coefficient of $(-1)^{k}\lambda^{n-k}$ in the characteristic polynomial of $A$:
...
\begin{remark}
	If we consider a complex vector bundle $(V, J)$ of $\dim_{\CC}(V) = n$ then, by forgetting the complex structure on $V$, we get an oriented real vector bundle $V$ of real dimension $\dim_{\RR}(V) = 2n$.
