...
\begin{document}
	...
	\begin{example}[\cite{Pestun2017}]\label{residue:5}
		Let us show that
		\begin{equation*}
			s_{n}(z_{0}, \ldots, z_{n}) = \sum_{k = 0}^{m} \frac{z_{k}^{n}}{\prod_{j \neq k} \pbrackets{ 1 - (z_{j} / z_{k}) }},
		\end{equation*}
		where $s_{n}(z_{0}, \ldots, z_{m})$ are complete homogeneous symmetric polynomials. Focus on the $m$-th term, relabel $z_{m} = x$, then the series above becomes
		\begin{equation*}
			\frac{x_{m}^{n}}{\prod_{j = 0}^{m-1} \pbrackets{1 - (z_{j}/x)}} = \frac{x_{m}^{n + m}}{(x - z_{0})\ldots (x - z_{m-1}) }
		\end{equation*}
	\end{example}
