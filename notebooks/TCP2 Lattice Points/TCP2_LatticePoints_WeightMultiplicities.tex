...
\begin{tikzpicture}[line cap=round,line join=round,>=triangle 45,x=1cm,y=1cm, scale=0.6]
	\begin{axis}[
		...
		\begin{scriptsize}
			...
			\draw[color=uuuuuu] (1.5,2.5) node {$\mathbf{z}_{123}^{\mathbf{k}}$}; % Delta123
			
			\draw[color=uuuuuu] (9,9) node {$\mathbf{z}_{12}^{\mathbf{k}}\cdot w_{3}^{a}$}; % Delta 12
			
			\draw[color=uuuuuu] (-11,7) node {$\mathbf{z}_{23}^{\mathbf{k}}\cdot w_{1}^{a}$}; % Delta 23

			\draw[color=uuuuuu] (7,-9) node {$\mathbf{z}_{13}^{\mathbf{k}}\cdot w_{2}^{a}$}; % Delta 13

			\draw[color=uuuuuu] (17,-6) node {$z_{1}^{k+a} \cdot \mathbf{w}_{23}^{\mathbf{a}}$}; % Delta 1
			
			\draw[color=uuuuuu] (-7,-7) node {$z_{2}^{k+a} \cdot \mathbf{w}_{13}^{\mathbf{a}}$}; % Delta 2
			
			\draw[color=uuuuuu] (-6,17) node {$z_{3}^{k+a} \cdot \mathbf{w}_{12}^{\mathbf{a}}$}; % Delta 3			
